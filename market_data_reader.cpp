 ...
 */
Symbol parse_symbol(std::string_view symbol_string)
{
    if (symbol_string == "SYNTH1")
    {
        return Symbol::SYNTH1;
    }
    else if (symbol_string == "SYNTH2")
    {
        return Symbol::SYNTH2;
    }
    else if (symbol_string == "SYNTH3")
    {
        return Symbol::SYNTH3;
    }
    else if (symbol_string == "SYNTH4")
    {
        return Symbol::SYNTH4;
    }
    else if (symbol_string == "SYNTH5")
    {
        return Symbol::SYNTH5;
    }
    else
    {
        throw std::invalid_argument(std::format("Cannot parse symbol: {}", symbol_string));
    }
}

Aggressor parse_aggressor(std::string_view aggressor_string)
{
    if (aggressor_string == "B")
    {
        return Aggressor::BUY;
    }
    else if (aggressor_string == "S")
    {
        return Aggressor::SELL;
    }
    else
    {
        throw std::invalid_argument(std::format("Cannot parse aggressor: {}", aggressor_string));
    }
}
...
std::vector<TradeData> parse_trade_data_from_csv(std::string_view csv_path)
{
    ...
    while (std::getline(file, line))
    {
        ...
        // Ignore bid price, bid quantity, ask price, ask quantity
        std::getline(line_stream, field, ',');
        std::getline(line_stream, field, ',');
        std::getline(line_stream, field, ',');
        std::getline(line_stream, field, ',');
        ...
        std::getline(line_stream, field, ',');
        Quantity quantity;
        std::from_chars(field.data(), field.data() + field.size(), quantity);

        // Extract aggressor
        std::getline(line_stream, field, ',');
        Aggressor aggressor{parse_aggressor(field)};

        trades.push_back(TradeData{timestamp, symbol, price, quantity, aggressor});
    }
